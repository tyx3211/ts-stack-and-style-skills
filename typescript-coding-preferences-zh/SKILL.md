...
这里采用的 TypeScript 风格是：边界以 schema（运行时校验结构）为先，流程以函数 / 模块为先，建模以数据为先，并且有意识地减少面向对象编程（OOP，面向对象编程）的默认比重。class（类）只作为局部工具使用，主要服务于 entity / value object（实体 / 值对象），而不是整个架构的默认基石。

## 默认立场

- 优先显式数据流、普通函数、小模块和组合，而不是继承。
...
- 如果仓库已经有明确框架约定，应当尊重本地惯例；但在没有本地理由时，不要额外引入更重的模式。

## 运行时与 TSConfig 默认项

- 新的服务端 TypeScript 项目，按部署目标优先 Node.js 24 或当前稳定版 Bun。
- 如果项目不是明确的 CommonJS-only（仅 CommonJS）形态，优先使用面向 ESM 的 TypeScript 配置：`module: "NodeNext"` 与 `moduleResolution: "NodeNext"`。
...
在修改 ESLint、hooks、CI 门禁、生成代码 lint 策略、`any` / `unknown`、断言、type guard、schema parse、相等性检查或数值校验时，请读取 [references/lint-hooks-type-safety.md](references/lint-hooks-type-safety.md)。
